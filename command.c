...
int save_statement(user *currentuser,int tc,char *cb[BUF_SIZE][BUF_SIZE]);
...
struct {
 ...
 unsigned int (*call_statement)(user *,int,void *);		/* function pointer */
		   ...
		   {  NULL,"SETCLASS",&setclass_statement },\
...
int save_statement(user *currentuser,int tc,char *cb[BUF_SIZE][BUF_SIZE]) {
  updateuser(currentuser,"","",currentuser->room,0,"",0,0,0,0,"","",0);
}
