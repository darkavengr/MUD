...
while((char) *strptr != 0) {			/* until end */
	if(((char) *strptr >= 'a') && ((char) *strptr <= 'z')) {
		*outptr++=*strptr-32;			/* to uppercase */
	}	
	else
	{
		*outptr++=*strptr;
...
strncpy(mmask,mask,BUF_SIZE);
strncpy(nname,filename,BUF_SIZE);
